...
inference. For the empirical-local learner, primitive fixed-bandwidth empirical-
process conditions imply the required nuisance rate; the corresponding WGAN
rate remains an explicit condition to be diagnosed. A covariance-aware
Richardson combination provides a separate route
...
fixed-bandwidth target, and the paper verifies the required rate for that
learner under a local overlap condition. For the exact reserve, a stable
$n^{-0.2}$ bandwidth
